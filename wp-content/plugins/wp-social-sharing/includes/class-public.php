...
class SS_Public {
	...
	public function social_sharing( $atts=array() ) {
		extract(shortcode_atts(array(
				...
				'email_text'    =>__('Email', 'social-sharing'),
				'icon_order'    =>'f,t,l,p,x,r,e',
				'social_image'  => '', 
				'show_icons'    =>'0',
				'before_button_text'=>'',
				'text_position'=> 'left'
	        ...
	        foreach($icon_order as $o) {
	        	switch($o) {
	        		case 'f':
	        			...
	        			?><a <?php echo $loadjs;?> rel="external nofollow" class="fab fa-facebook-square" href="http://www.facebook.com/sharer/sharer.php?u=<?php echo $url; ?>" target="_blank" ><?php //echo $facebook_text; ?></a><?php
	        			}
	        		break;
	        		case 't':
	        			if(in_array('twitter', $social_options)){
	        			?><a <?php echo $loadjs;?> rel="external nofollow" class="fa fa-twitter-square" href="http://twitter.com/intent/tweet/?text=<?php echo $title; ?>&url=<?php echo $url; ?><?php if(!empty($twitter_username)) {  echo '&via=' . $twitter_username; } ?>" target="_blank"><?php //echo $twitter_text; ?></a><?php
	        			}
	        		break;
					case 'l':
						if(in_array('linkedin', $social_options)){
							?><a <?php echo $loadjs;?> rel="external nofollow" class="fa fa-linkedin-square" href="http://www.linkedin.com/shareArticle?mini=true&url=<?php echo substr($url,0,1024);?>&title=<?php echo substr($title,0,200);?>" target="_blank" ><?php //echo $linkedin_text; ?></a><?php
						}
	        		break;
	        		case 'p':
	        			if(in_array('pinterest', $social_options)){
	        				?><a <?php echo $loadjs;?> rel="external nofollow" class="fa fa-pinterest-square" href="http://pinterest.com/pin/create/button/?url=<?php echo $url;?>&media=<?php echo $social_image;?>&description=<?php echo $title;?>" target="_blank" ><?php //echo $pinterest_text; ?></a><?php
	        			}
					break;
                    case 'x':
					...
					case 'r':
						if(in_array('reddit', $social_options)){
							?><a <?php echo $loadjs;?> rel="external nofollow" class="fa fa-pinterest-p" href="http://reddit.com/submit?url=<?php echo $url;?>&amp;title=<?php echo $title?>" target="_blank"><?php echo $reddit_text;?></a><?php
                        }
					break;
					case 'e':
						if(in_array('email', $social_options)){?>
						<a <?php echo $loadjs;?> rel="external nofollow" class="fa fa-envelope" href="mailto:type email address here?subject=I wanted to share this post with you from <?php bloginfo('name'); ?>&body=<?php the_title('','',true); ?>&#32;&#32;<?php the_permalink(); ?>&amp;title=<?php echo $title?>" target="_blank">
							<?php //echo $email_text;?>
						</a>
						<?php }
                    break;
